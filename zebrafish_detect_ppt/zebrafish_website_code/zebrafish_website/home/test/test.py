...
def creatcsv(csv_name,count_rate):
    count_rate_with_percentage = [
        [str(data) + " (%)" for data in row] for row in count_rate
    ]  # 將每一個資料後都加上(%)
    file_path = os.path.join("media/csv_file", f"{csv_name}.csv")#將寫好的csv檔傳入指定資料夾
    ...
    with open(file_path, "w", newline="") as file:
        ...
        for i, row in enumerate(count_rate_with_percentage):
            writer.writerow([str(i + 1)] + row)  # 加入編號,再寫入資料

def save_img(upload_img,detect_img,upload_img_name,detect_img_name):
    cv2.imwrite(f"media/upload/{upload_img_name}",upload_img)
    cv2.imwrite(f"media/detect/{detect_img_name}.jpg",detect_img)
    


# origin_img, gray_img = load_img(
#     "20220616_Dex+naringin_with 4X P_2.5X-20230505T092937Z-001\\20220616_Dex+naringin_with 4X P_2.5X\\1X E3\\1X E3 -40.jpg"
# )  # 11,21,30出問題
class main:
    def m(file):
        global origin_img
        origin_img = upload_img = cv2.imdecode(np.fromstring(file.read(), np.uint8), cv2.IMREAD_COLOR)

        origin_img = cv2.resize(origin_img, (920, 540)) 
        ...
        #     print(row)
        csv_name = (file.name + '_data')
        image_name = (file.name + '_detect')
        creatcsv(image_name,count_rate)
        save_img(upload_img,image,file.name,image_name)
